...
SELECT
  gt.block_time AS launch_block_time,
  CASE 
...
FROM
  graduated_tokens AS gt
  LEFT JOIN token_info AS ti ON gt.token_mint_address = ti.token_mint_address
  LEFT JOIN dev on gt.token_mint_address = dev.token_mint_address
  LEFT JOIN holders AS h ON gt.token_mint_address = h.token_mint_address
  LEFT JOIN latest_prices AS lp ON gt.token_mint_address = lp.token_mint_address
  AND lp.rn = 1
  LEFT JOIN token_supplies AS ts ON gt.token_mint_address = ts.token_mint_address
ORDER BY
  holder_count DESC